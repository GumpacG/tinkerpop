...
[llms-summary="Provider documentation overview: what graph system providers, graph driver providers, and language variant providers must implement to integrate with TinkerPop."]
= Introduction
...
[[providers]]
[llms-summary="Guidance for those building on TinkerPop: implementing a graph system, a driver, or a language variant, and validating it against the test suite."]
...
[llms-summary="What a graph system provider must implement: the structure/process APIs, OLTP and OLAP support, and how to validate the implementation with the Gremlin test suite."]
== Graph System Provider Requirements
...
A custom GraphBinary serializer reads bytes that may not be trusted, so any length or element count it reads from the
`Buffer` must be validated before it is used to size an allocation or bound a read loop. A crafted message can declare
a count far larger than the bytes it actually carries, and allocating on that count first turns a few bytes into an
arbitrarily large allocation. Since every counted element or byte needs at least one byte on the wire, a count that
exceeds `Buffer.readableBytes()` cannot be satisfied and should be rejected with an `IOException`. The serializers
TinkerPop ships do this, but the check is internal to them, so an implementation of `CustomTypeSerializer` needs its
own:

[source,java]
----
public MyType read(final Buffer buffer, final GraphBinaryReader context) throws IOException {
    final int length = buffer.readInt();
    if (length < 0 || length > buffer.readableBytes())
        throw new IOException("Invalid length prefix for MyType: " + length);

    // only now is it safe to allocate against length
}
----

Prefer growing a collection as elements are read over pre-sizing it to the declared count, and treat a count read as a
`long` (a repeat count, for example) the same way, since a large positive value can drive unbounded work even when no
...
[llms-summary="Using the gremlin-test suite to validate a graph implementation: the structure and process test standards and how to opt in or out of feature-gated tests."]
=== Validating with Gremlin-Test
...
[llms-summary="What a driver provider must implement to connect a language to Gremlin Server: the wire protocol, serialization formats, and connection handling."]
...
[[gremlin-plugins]]
[llms-summary="How to author Gremlin Console/Server plugins as a provider: the plugin interface, dependency management, and registering custom bindings and DSLs."]
== Gremlin Plugins
