...
This can be an issue in situations where the application calling Gremlin-Python is already using an event loop. To support
this case, passing `call_from_event_loop=True` to the `AiohttpTransport` class causes the transport to run its own event loop
on a dedicated background thread and dispatch requests onto it via `asyncio.run_coroutine_threadsafe`, avoiding errors like
